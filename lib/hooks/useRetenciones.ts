...
import type { RetencionesTipo, RetencionPorPersona, CreateRetencionPorPersonaDto } from '@/lib/types/retenciones'
  ...
  const fetchTipos = useCallback(async () => {
    ...
    try {
      const result = await apiGet<RetencionesTipo[]>(
        `/api/retenciones-tipos?soloActivos=${soloActivos}`
      )
      setTipos(
...
export function useRetencionesPorPersona(terceroId?: number) {
  ...
  useEffect(() => { fetchRetenciones() }, [fetchRetenciones])

  const agregar = async (
    dto: CreateRetencionPorPersonaDto
  ): Promise<boolean> => {
