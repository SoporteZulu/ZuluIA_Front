...
import type { Comprobante, ComprobanteDetalle, EmitirComprobanteDto, EstadisticasComprobantes, TipoComprobante } from '@/lib/types/comprobantes'
...
export function useComprobantes(options: UseComprobantesOptions = {}) {
  ...
  const fetchComprobantes = useCallback(async () => {
    ...
    try {
      ...
      if (options.tipoComprobanteId) params.set('tipoComprobanteId', String(options.tipoComprobanteId))
  ...
  }, [page, estado, options.tipoComprobanteId, options.terceroId, options.sucursalId, needsTypeFilter])
  ...
  const asignarCae = async (id: number, cae: string, fechaVto: string, qrData?: string): Promise<boolean> => {
    try {
...
export function useComprobantesConfig() {
  const [tipos, setTipos] = useState<{ id: number; codigo: string; descripcion: string; esVenta: boolean; esCompra: boolean }[]>([])
  const [estados, setEstados] = useState<{ valor: string; descripcion: string }[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    Promise.all([
      apiGet<{ id: number; codigo: string; descripcion: string; esVenta: boolean; esCompra: boolean }[]>('/api/comprobantes/tipos'),
