...
import type {
  HDTicket, HDTicketComment, HDServicio, HDOrdenServicio,
  HDSLA, HDContrato, HDAgente, HDCliente, HDFacturaServicio,
} from '@/lib/types'
...
export function useHdTickets() {
  ...
  async function addComment(ticketId: string, dto: Omit<HDTicketComment, 'id' | 'createdAt' | 'updatedAt'>) {
    return apiPost<HDTicketComment>(`/api/helpdesk/tickets/${ticketId}/comentarios`, dto)
  }

  return { tickets, loading, error, search, setSearch, createTicket, updateTicket, deleteTicket, addComment, refetch: fetchTickets }
}
...
export function useHdAgentes() {
  ...
  return { agentes, loading, error, createAgente, updateAgente, deleteAgente, refetch: fetchAgentes }
}
...
export function useHdContratos() {
  ...
  const fetchContratos = useCallback(async () => {
    try {
      setLoading(true)
      setError(null)
      const res = await apiGet<HDContrato[] | PagedResult<HDContrato>>('/api/helpdesk/contratos?size=200')
  ...
  return { contratos, loading, error, createContrato, updateContrato, deleteContrato, refetch: fetchContratos }
}
...
export function useHdServicios() {
  ...
  const fetchServicios = useCallback(async () => {
    try {
      setLoading(true)
      setError(null)
      const res = await apiGet<HDServicio[] | PagedResult<HDServicio>>('/api/helpdesk/servicios?size=200')
  ...
  return { servicios, loading, error, createServicio, updateServicio, deleteServicio, refetch: fetchServicios }
}
...
export function useHdClientes() {
  ...
  const fetchClientes = useCallback(async () => {
    try {
      setLoading(true)
      setError(null)
      const res = await apiGet<HDCliente[] | PagedResult<HDCliente>>('/api/helpdesk/clientes?size=200')
...
export function useHdFacturacion() {
  ...
  const fetchFacturas = useCallback(async () => {
    try {
      setLoading(true)
      setError(null)
      const res = await apiGet<HDFacturaServicio[] | PagedResult<HDFacturaServicio>>('/api/helpdesk/facturas?size=200')
  ...
  return { facturas, loading, error, createFactura, updateFactura, deleteFactura, refetch: fetchFacturas }
}
