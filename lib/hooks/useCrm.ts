...
import type {
  CRMClient, CRMContact, CRMOpportunity, CRMInteraction,
  CRMTask, CRMCampaign, CRMSegment, CRMUser,
} from '@/lib/types'
...
export function useCrmClientes() {
  ...
  return { clientes, loading, error, search, setSearch, createCliente, updateCliente, deleteCliente, refetch: fetchClientes }
}
...
export function useCrmContactos(clienteId?: string) {
  ...
  return { contactos, loading, error, createContacto, updateContacto, deleteContacto, refetch: fetchContactos }
}
...
export function useCrmOportunidades(clienteId?: string) {
  ...
  return { oportunidades, loading, error, createOportunidad, updateOportunidad, deleteOportunidad, refetch: fetchOportunidades }
}
...
export function useCrmInteracciones(clienteId?: string) {
  ...
  return { interacciones, loading, error, createInteraccion, deleteInteraccion, refetch: fetchInteracciones }
}
...
export function useCrmCampanas() {
  ...
  const fetchCampanas = useCallback(async () => {
    try {
      setLoading(true)
      setError(null)
      const res = await apiGet<CRMCampaign[] | PagedResult<CRMCampaign>>('/api/crm/campanas?size=200')
  ...
  return { campanas, loading, error, createCampana, updateCampana, deleteCampana, refetch: fetchCampanas }
}
...
export function useCrmSegmentos() {
  ...
  const fetchSegmentos = useCallback(async () => {
    try {
      setLoading(true)
      setError(null)
      const res = await apiGet<CRMSegment[] | PagedResult<CRMSegment>>('/api/crm/segmentos?size=200')
  ...
  useEffect(() => { fetchSegmentos() }, [fetchSegmentos])
  ...
  return { segmentos, loading, error, createSegmento, updateSegmento, deleteSegmento, refetch: fetchSegmentos }
}
